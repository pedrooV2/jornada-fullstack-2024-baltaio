    ...
    public class CategoryHandler(IHttpClientFactory httpClientFactory) : ICategoryHandler
    {
        ...
        public async Task<PagedResponse<List<Category>?>> GetAllAsync(GetAllCategoriesRequest request)
        {
            return await _client.GetFromJsonAsync<PagedResponse<List<Category>?>>("v1/categories")
                ?? new PagedResponse<List<Category>?>(null, 400, "Falha ao consultar uma categoria");
        }

        public async Task<Response<Category?>> GetByIdAsync(GetCategoryByIdRequest request)
        {
            return await _client.GetFromJsonAsync<Response<Category?>>($"v1/categories/{request.Id}")
                ?? new Response<Category?>(null, 400, "Falha ao consultar uma categoria");
        }
        ...
        public async Task<Response<Category?>> UpdateAsync(UpdateCategoryRequest request)
        {
            ...
            return await result.Content.ReadFromJsonAsync<Response<Category?>>()
                ?? new Response<Category?>(null, 400, "Falha ao editar categoria");
        }
    }
}